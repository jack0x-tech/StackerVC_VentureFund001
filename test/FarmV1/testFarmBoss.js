...
contract("test FarmTreasuryV1", async (accounts) => {
	...
	it("should add/remove whitelist", async () => {
		...
		await _farmboss.changeWhitelist([{account: USER, fnSig: FALLBACK_FN}], [], [{token: _underlying.address, allow: USER}], [], {from: GOVERNANCE});

		assert(await _farmboss.getWhitelist(USER, FALLBACK_FN));
		assert(! await _farmboss.getWhitelist(USER_2, FALLBACK_FN));
	...
	it("should add whitelist, then allow farmer execute", async () => {
		...
		await _farmboss.changeWhitelist([{account: _underlying.address, fnSig: depositWETH},{account: _underlying.address, fnSig: withdrawWETH},{account: USER_2, fnSig: FALLBACK_FN}], [], [{token: _underlying.address, allow: USER}], [], {from: GOVERNANCE});
